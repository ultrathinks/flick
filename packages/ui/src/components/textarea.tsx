...
export function Textarea({ label, error, help, className, ...props }: Props) {
  const field = (
    <textarea
      ...
      className={cn(
        "min-h-24 w-full resize-none rounded-control border bg-surface px-4 py-3 text-heading text-foreground outline-none transition-colors placeholder:text-foreground-faint focus:ring-2",
        error
