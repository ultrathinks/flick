...
- Treat `AGENTS.md`, `AGENTS.md`, and explicit user instructions as higher priority than this skill.
...
5. Commit message format: follow active repo guidance (`AGENTS.md` / `AGENTS.md` / commit config), else recent local style if consistent, else the **Commit message rules** below. flick's history already uses `type: description` lowercase (feat/fix/chore/docs), which matches those rules.
